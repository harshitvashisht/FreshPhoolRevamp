...
export default function PayIndexPage() {
  ...
  return <main className="wrap">
    ...
    <h1>{invoice ? "Payment complete" : cashOnDelivery ? "Order placed" : "Choose payment"}</h1>
    <p>{invoice ? `Invoice ${invoice.invoiceNumber} has been created for your confirmed order.` : cashOnDelivery ? "Cash on Delivery selected. Please keep the exact amount ready; your invoice will be issued after collection." : "Choose secure online payment with Razorpay or pay cash when your order is delivered."}</p>
    ...
    {expiryNotice && !invoice && !cashOnDelivery ? <p className="fp-muted">{expiryNotice}</p> : null}
    <div className="actions">{invoice ? <><button className="btn btn-primary" type="button" onClick={() => window.print()}>Download invoice</button><a className="btn btn-ghost" href="/dashboard/orders">View my orders</a></> : cashOnDelivery ? <><a className="btn btn-primary" href="/dashboard/orders">View my orders</a><a className="btn btn-ghost" href="/">Continue shopping</a></> : <><button className="btn btn-primary" type="button" disabled={!order || busy} onClick={pay}>{busy ? "Opening Razorpay…" : order ? `Pay online · ${rupee(order.subtotalRupee)}` : "Loading…"}</button><button className="btn btn-ghost" type="button" disabled={!order || busy} onClick={chooseCashOnDelivery}>{busy ? "Saving…" : "Cash on Delivery"}</button><a className="btn btn-ghost" href="/">Back to storefront</a></>}</div>
  </main>;
}
